...
2. Use the attached OpenClaw repo folders, especially `docs/`, `src/`, and `extensions/`, as current reference material only when implementing related features.
...
4. Put Mermaid diagrams under `docs/specs/diagrams`.
5. Put system-behavior and how-it-works specifications under `docs/specs`.
6. Put fix recaps and “how it works now” notes under `docs/fixes`.
7. Use the filename format `timestamp-description-filename.md` for these docs so entries stay sortable and easy to scan.

### Markdown Diagram Rules

When a request asks for a diagram inside a Markdown document, use Mermaid unless another format is explicitly requested.

1. Always return diagrams inside a fenced `mermaid` block that is ready to paste into a `.md` file.
2. Prefer simple, readable diagrams over dense ones, and split complex diagrams into smaller pieces when needed.
3. Keep labels short and avoid decorative text, long sentences, and unnecessary styling.
4. Choose the right Mermaid type for the request: sequence diagrams for interactions, flowcharts for workflows and decisions, state diagrams for lifecycle changes, class diagrams for domain structure, ER diagrams for database relationships, and Gantt charts only when a timeline is clearly useful.
5. In sequence diagrams, show participants in logical order, keep the main path only unless an alternate path matters, use `alt`, `opt`, and `loop` only when they improve clarity, and do not invent systems or steps that were not described.
6. In flowcharts, use top-down flow by default, make decisions explicit, and avoid crossing lines when possible.
7. Do not explain Mermaid syntax unless asked, do not wrap the Mermaid block in another code block, and do not use unsupported Mermaid features.
8. If details are missing, make the safest minimal assumption and keep the diagram generic.
9. When responding with a diagram, keep the output to a short title, the Mermaid block, and an optional brief note only if needed.
